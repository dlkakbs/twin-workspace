...
def _load_heygen_settings():
    ...
    return TwinSettings(
        ...
        did_api_key=env.get("DID_API_KEY"),
        heygen_api_key=env.get("HEYGEN_API_KEY"),
        avatar_provider=env.get("TWIN_AVATAR_PROVIDER", "heygen").strip().lower(),
        output_root=contract.output_root,
        did_base_url=env.get("DID_BASE_URL", "https://api.d-id.com"),
        ...
        kimi_base_url=env.get("KIMI_BASE_URL", "https://api.moonshot.ai/v1"),
        kimi_profile_model=env.get("TWIN_KIMI_PROFILE_MODEL", "moonshot-v1-128k"),
        kimi_generation_model=env.get("TWIN_KIMI_GENERATION_MODEL", "moonshot-v1-128k"),
        ...
        did_poll_interval_seconds=float(env.get("TWIN_DID_POLL_INTERVAL", "3")),
        did_timeout_seconds=int(env.get("TWIN_DID_TIMEOUT", "240")),
